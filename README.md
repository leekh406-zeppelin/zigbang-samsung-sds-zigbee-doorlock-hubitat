# Zigbang(Samsung SDS) Zigbee Door Lock Driver for Hubitat

Hubitat 커스텀 드라이버 — 직방(삼성SDS) 도어락(SHP-DP960 / SHP-960 Plus, Zigbee 모듈 DADT302)을
...
- **배터리 잔량 확인 불가 → v9부터 100%로 고정 표시.** 표준 배터리 % 속성(`0x0021`)은 미지원
  (status 0x86) 응답이 오고, 전압 속성(`0x0020`)은 Configure Reporting은 성공하지만 실제 값이
  항상 `0`으로 리턴됩니다. ZDO 바인딩을 명시적으로 재시도해봐도 동일했습니다. 원본 엣지드라이버
  개발자가 커뮤니티에서 확인해준 바로는, WiFi 모듈 경로로도 신뢰할 만한 값을 얻지 못하는데
  잠금/해제마다 모터 전류로 전압이 순간적으로 요동치기 때문이라고 합니다. 삼성 공식 DTH도
  battery=100 하드코딩. → **배터리 관리는 도어락 본체의 저전압 경고(LED/음성 안내)에 의존하는
  것을 권장합니다.**
...
2. 직방 도어락 전면의 초기화 홀에 핀으로 꼽아 기기 초기화를 하였습니다.

3. 베터리 측 커버를 열고 등록 버튼을 1번 누르고 -> 비밀번호* -> 4 * -> 무선연동( 1*)을 누른 상태에서 허비탯과 페어링

4. device info 메뉴 - > type에서 드라이버 변경 후 save

5.  commands 메뉴 - > configure 한번 클릭 후 사용
...
- **v8**: v7 적용 후 "Hubitat에서 Unlock 눌러도 Current States에 반영 안 됨" 버그 발견 →
  원인은 제조사 전용 커맨드(0x1F)로 언락할 때 도어락이 보내는 Operation Event의
  operationEventCode가 표준 코드가 아닌 **확장 코드 0x10(16)**이었던 것. 기존 코드 매핑에
  0x10이 없어서 이벤트가 조용히 무시되고 있었음 → 코드 16→unlocked, 17→locked(방어적 추가)
  매핑 추가로 해결. 키패드/지문/자동잠금은 원래부터 표준 코드라 문제없었고, **원격 Unlock
  경로만** 영향받았던 것으로 확인됨.
- **v9**: 배터리 %를 전압 기반 계산 대신 **100으로 고정**. 원본 엣지드라이버 개발자(스피드박님)가
  커뮤니티 댓글로 확인해준 내용: WiFi 모듈(직방앱) 경로로도 신뢰할 만한 배터리 값을 얻을 수
  없는데, 이유는 도어락이 잠금/해제할 때마다 모터에 큰 전류가 흘러 전압이 순간적으로
  떨어졌다가 회복되기 때문. 측정 시점마다 값이 크게 흔들려서 감소 추세조차 안정적으로 보여줄
  수 없음. 삼성 공식 DTH도 동일하게 battery=100 하드코딩. 틀린/불안정한 숫자를 보여주는 것보다
  100%로 고정하고 실제 방전 경고는 도어락 자체 알림(음성/LED)에 맡기는 것이 낫다고 결론.
  `batteryVoltage`는 참고용으로 계속 제공.
