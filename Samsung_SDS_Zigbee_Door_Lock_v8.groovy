/**
 *  Samsung SDS Zigbee Door Lock Driver for Hubitat (v8)
 *  Supports: SHP-960 Plus, SHP-DP960SG (DADT302 Zigbee module)
 *  Manufacturer: SAMSUNG SDS
 *
 *  v8 changes - critical fix:
 *   - Found via logs why v7's manufacturer-specific unlock() sometimes never
 *     updated the "lock"/"contact" attributes: when unlocked via our
 *     manufacturer-specific virtual-keypad command (0x1F), the device's own
 *     Operation Event Notification push uses operationEventCode 0x10 (16) -
 *     an EXTENDED code outside the standard ZCL set (1/2/7/8/9/10/13/14)
 *     that v7's codeMap didn't recognize, so the event was silently dropped
 *     (returned null). Meanwhile keypad/fingerprint/autolock events use the
 *     standard codes and worked fine - only the remote-unlock path was
 *     affected. Combined with v7 intentionally ignoring readAttribute
 *     results, this meant the "lock" attribute could get stuck stale right
 *     after 상화/오빠 triggered an unlock via Hubitat.
 *   - Added code 16 (0x10) -> unlocked, and defensively 17 (0x11) -> locked
 *     (symmetric guess for a possible "remote lock" extended code, not yet
 *     observed in logs but added defensively since our lock() could
 *     plausibly trigger an analogous extended code someday).
 *
 *  v7 changes:
...
metadata {
    definition(name: "Samsung SDS Zigbee Door Lock v8", namespace: "community", author: "community") {
        capability "Lock"
...
def parseOperationEvent(List data) {
    ...
    switch (operationEventCode) {
        ...
        // v8: extended/manufacturer-specific codes seen when unlocking via
        // the manufacturer-specific virtual-keypad command (0x1F). Code 16
        // (0x10) was confirmed in logs to fire right after such an unlock.
        // Code 17 (0x11) is added defensively as a symmetric guess for a
        // possible analogous "remote lock" case - not yet observed, but
        // harmless if it never occurs.
        case 16:
            lockState = "unlocked"; break
        case 17:
            lockState = "locked"; break
