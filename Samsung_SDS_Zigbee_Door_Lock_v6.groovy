/**
 *  Samsung SDS Zigbee Door Lock Driver for Hubitat (v6)
...
metadata {
    definition(name: "Samsung SDS Zigbee Door Lock v6", namespace: "community", author: "community") {
        capability "Lock"
...
def parseReadAttr(String description) {
    ...
    // Door Lock cluster 0x0101, attr 0x0000 (lock state)
...
def parseLockState(String value) {
    if (!value) return null
    ...
    if (txtEnable) log.info "${device.displayName} lock is ${lockState}"
    def events = []
    events << createEvent(name: "lock", value: lockState, descriptionText: "${device.displayName} is ${lockState}")
